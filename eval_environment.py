...
def ssh_remote_subprocess(recv_command, server_user, server_ip, seconds=1):
    # subprocess.Popen("ssh -t " + server_user + "@" + server_ip + " " + recv_command, shell=True)
    os.system("ssh " + server_user + "@" + server_ip + " " + recv_command +
              " & > /home/hkucs/qemu_output/log-" + server_ip + "-" + getTimeStamp())
    time.sleep(seconds)
...
def ssh_remote_nc(recv_command, server_user, server_ip, telnet_ip, telnet_port, seconds=1):
    os.system("ssh -t " + server_user + "@" + server_ip +
              ' echo "' + recv_command +
...
if __name__ == "__main__":
    ...
    parser.add_argument('--type', '-t', choices=('mc', 'colo'),
                        type=str,
    ...
    if type == "mc":
        PRIMARY_COMMAND = getConfigSections(local_config, "KVM_MC")['primary_command']
        SECONDARY_COMMAND = getConfigSections(local_config, "KVM_MC")['secondary_command']
    ...
    if mode == "start":
        if type == "mc":
            try:
                command = "ssh " + PRIMARY_IP + " sudo " + PRIMARY_QEMU_PATH + PRIMARY_COMMAND
                command = "sudo nohup /home/hkucs/qemumc/qemu/x86_64-softmmu/qemu-system-x86_64 " \
            ...
            # try:
                ...
                t1 = threading.Thread(target=ssh_remote_subprocess_master,
                                      args=(command, PRIMARY_USERNAME, PRIMARY_IP))
                t1.start()
                time.sleep(5)
                ...
                t2 = threading.Thread(target=ssh_remote_subprocess_slave,
                                      args=(command, SECONDARY_USERNAME, SECONDARY_IP))
                t2.start()
                time.sleep(5)
